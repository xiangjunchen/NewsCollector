...
②网易新闻数据
获取评论数
 #http://sdk.comment.163.com/api/v1/products/a2869674571f77b5a0867c3d71db5856/threads/BTV1A25D00014SEH
 #http://sdk.comment.163.com/api/v1/products/a2869674571f77b5a0867c3d71db5856/threads/BTKA08A900234IG8
 评论链接
#pinglun_=http://comment.news.163.com/news3_bbs/AU6BAQ8000011229.html;
    ...
    def ParsePage(self,url):
        try:
            ...
            if html:
                ...
                #栏目
                lanmu = tree.xpath('.//div[@class="post_crumb"]/a/text()')
                #标题
                biaoti = self.textxpath(tree,'.//head/title/text()')
                #关键词
                guanjianci = self.textxpath(tree,'.//head/meta[@name="keywords"]/@content')
                #发布时间
                shijian = self.textxpath(tree,'.//div[@class="post_time_source"]/text()')
                timestamp=0
                if shijian:
                    shijian = re.findall('\d{4}-\d{1,2}-\d{1,2} \d{2}:\d{2}:\d{2}',shijian)[0]
                    timestamp = self.datatransform(shijian)
                #文章来源
                laiyuan = self.textxpath(tree,'.//div[@class="post_time_source"]/a/text()')
                #正文
                wen_zhang_zheng_wen = xpathutil.get_Node_text(tree,'.//div[@class="post_text"]/p')
                #图片链接
                tu_pian_lian_jie = tree.xpath('.//div[@class="post_text"]/p/img/@src')
                #评论数
    ...
    def GetPinglun(self,url):
        ...
        try:
            ...
            if data["tcount"]!=0:
                ...
                uid =data["docId"]
                # print uid
                comment_url = 'http://comment.news.163.com/news3_bbs/%s.html'%uid
                # print comment_url
